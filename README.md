...
### 3. 密钥自动提取与高亮大字报
- **CPA 安装完成**：自动提取并在终端以高亮表格输出 **CPA Management Key** 与服务端口，方便后续直接对接面板。
- **CPAMP 安装完成**：自动提取并在终端高亮输出 **CPAMP 管理员密钥** 与登录地址，避免在繁杂的日志中苦苦翻找。
- **全部安装完成**：自动汇总两大核心凭据，生成一目了然的凭据清单。
...
- **灵活备份策略**：
  ...
  - **无备份直接升级 (小硬盘深度瘦身模式)**：
    - 升级时不生成额外的历史快照副本；
    - 升级成功并完成健康检查后，**自动清理 `runtime/` 历史版本残留目录**（只保留当前最新版本）；
    - **自动清空 `downloads/` 安装包缓存**，彻底释放硬盘空间！
- **5 级高容错路径探测**：
